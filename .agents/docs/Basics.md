...
For general questions about the Dendritic Pattern, refer to the [FAQ](FAQ.md) section.
...
- External code (e.g., `hardware.nix`) should be extended with the appropriate Flake-Parts definition to integrate it into our feature module system, which is the preferred approach. Alternatively, it can be placed outside the the central "modules" folder and included manually. For instance, when using custom packages in Flake-Part's `perSystem` with `pkgs.callPackage`.
